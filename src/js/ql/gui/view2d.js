...
export default class View2D extends Element {
	constructor(_conf, _scene, _editor) {
		super(_conf.dom);
		...
		$.fromEvent(dom, 'mousemove').subscribe(ev => {
			if (["mousedown", "mousemove"].indexOf(_view.interaction.status) > -1) {
				...
				changeVector.x = Number(changeVector.x / 2.5) * 2.5;
				changeVector.y = Number(changeVector.y / 2.5) * 2.5;
	...
	drawObject(canvas, obj, _lineDash, _strokeStyle, _showCoords) {
		var ctx = canvas.ctx;
		var _mod = this.mod;
		var scope = this;
		...
		const transformVectorTo2DCanvas = v3 =>
		new ext.Vector3().copy(v3).multiply(scaleVector)
			.applyEuler(obj.rotation).add(objPos3).toVector2(_mod)
			.multiplyScalar(scope.zoom / 100).add(center2);
		...
		if (obj.geometry.quads) {
			...
			// TODO: Impl a check whether a face should be displayed or not
				...
				var quad2d = {
					a: transformVectorTo2DCanvas(_quad.a),
					b: transformVectorTo2DCanvas(_quad.b),
					c: transformVectorTo2DCanvas(_quad.c),
					d: transformVectorTo2DCanvas(_quad.d)
				};
			...
			ctx.closePath();*/
				...
				if (_showCoords) {
					...
					if (scope.editor.indexes.length === 0 || scope.editor.indexes.indexOf(String(index)) > -1) {
